...
import tr.edu.iyte.dnmso.domain.Modification.ModAminoAcid;
import tr.edu.iyte.dnmso.domain.Modification.Terminal;
import tr.edu.iyte.dnmso.domain.Prediction.Sequence;
import tr.edu.iyte.dnmso.domain.Prediction.Sequence.Gap;
							...
							for (Clause spectrumClause : spectrumFrame.getClauses("property_value")){
                                if(spectrumClause.getValue2()!=null && spectrumClause.getValue()!=null ){
                                    ...
                                    if(spectrumClause.getValue().equals("hasPrecursorMZ")){
                                        spectrum.setPrecursorMass(Double.valueOf((String)spectrumClause.getValue2()));
                                    }
                                    if(spectrumClause.getValue().equals("hasScanId")){
                                        spectrum.setScanNumber((String)spectrumClause.getValue2());
                                    }
                                }
		...
		try {
			...
			for (MassSpectrum spectrum: dnmso.getSpectra().getSpectrum()){
				...
				if(spectrum.getLink()!=null){
                 ...
                 if(spectrum.getPrecursorMass()!=null) {
